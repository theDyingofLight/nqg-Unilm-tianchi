...
from pytorch_transformers.modeling_bert import load_tf_weights_in_bert, BertPooler, BertIntermediate, BertOutput, \
...
class UnilmModel(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, answer_tag=None, attention_mask=None,
                output_all_encoded_layers=True):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)

        embedding_output = self.embeddings(
            input_ids, token_type_ids, answer_tag)
...
class UnilmModelIncr(UnilmModel):
    ...
    def forward(self, input_ids, token_type_ids, answer_tag, position_ids, attention_mask,
                output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)

        embedding_output = self.embeddings(
            input_ids, token_type_ids, answer_tag, position_ids)
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, answer_tag=None, attention_mask=None, masked_lm_labels=None,
                masked_pos=None, masked_weights=None, num_tokens_a=None, num_tokens_b=None):
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, answer_tag, attention_mask, output_all_encoded_layers=False)
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, answer_tag, position_ids, attention_mask):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, answer_tag, position_ids, attention_mask)
        ...
        while next_pos < output_length:
            ...
            curr_answer_tag = answer_tag[:, start_pos:next_pos + 1]
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, curr_token_type_ids,curr_answer_tag, curr_position_ids, curr_attention_mask,
    ...
    def beam_search(self, input_ids, token_type_ids,answer_tag, position_ids, attention_mask):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        ...
        while next_pos < output_length:
            ...
            if is_first:
                ...
                answer_tag=first_expand(answer_tag)
