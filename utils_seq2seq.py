...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def read_data(self, line, tokenizer):
        ...
        ans_tk = sample["answer_text"]
        return (src_tk, tgt_tk, ans_tk)
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        tokens_a, tokens_b, tokens_ans = instance[:3]
        tokens_a = self.tokenizer.tokenize(tokens_a)
        tokens_b = self.tokenizer.tokenize(tokens_b)
        tokens_ans = self.tokenizer.tokenize(tokens_ans)
        # -3  for special tokens [CLS], [SEP], [SEP]
        tokens_a, tokens_b = truncate_tokens_pair(tokens_a, tokens_b, self.max_len)
        # Add Special Tokens
        tokens = ['[CLS]'] + tokens_a + ['[SEP]'] + tokens_b + ['[SEP]']
        segment_ids = [0] * (len(tokens_a) + 2) + [1] * (len(tokens_b) + 1)
        answer_tag = [0] * (len(tokens_a) + len(tokens_b) + 3)
        flag = 1
        for s in range(len(tokens)):
            if tokens[s] == tokens_ans[0]:
                for t in range(len(tokens_ans)):
                    if tokens[s + t] == tokens_ans[t]:
                        if t == len(tokens_ans) - 1:
                            for k in range(s, s + len(tokens_ans)):
                                answer_tag[k] = 2
                            flag = 0
                    else:
                        break
            ...
            if s == len(tokens) - 1:
                for t in range(len(tokens_ans)):
                    for m in range(len(tokens)):
                        if tokens[m] == tokens_ans[t]:
                            answer_tag[m] = 2
                            break
        for s in range(len(answer_tag)):
            if answer_tag[s] != 0:
                answer_tag[s] = 1
                break
        ...
        return (
            input_ids, segment_ids, answer_tag, input_mask, masked_ids, masked_pos, masked_weights, next_sentence_label)
...
class Preprocess4Seq2seqDecode(Pipeline):
    ...
    def __init__(self, vocab_words, indexer, max_len=512, max_tgt_length=128,max_passage_length=440, tokenizer=None):
        super().__init__()
        self.max_len = max_len
        self.vocab_words = vocab_words  # vocabulary (sub)words
        self.indexer = indexer  # function from token to token index
    ...
    def __call__(self, instance):
        ...
        tokens_a = self.tokenizer.tokenize(tokens_a)
        tokens_ans = self.tokenizer.tokenize(tokens_ans)
        max_a_len = self.max_passage_length

        # Add Special Tokens
        padded_tokens_a = ['[CLS]'] + tokens_a + ['[SEP]']
        if len(padded_tokens_a)>max_a_len+2:
        ...
        assert len(padded_tokens_a) == max_a_len + 2
        if self.max_tgt_length + len(padded_tokens_a) >= self.max_len:
            cut_num = self.max_len -len(padded_tokens_a)- self.max_tgt_length
            tokens_a = tokens_a[:cut_num]
            padded_tokens_a = ['[CLS]'] + tokens_a + ['[SEP]']
        max_len_in_batch = min(self.max_tgt_length +
                               len(padded_tokens_a), self.max_len)
        tokens = padded_tokens_a
        segment_ids = [0] * (len(padded_tokens_a)) + [1] * (max_len_in_batch - len(padded_tokens_a))

        answer_tag = [0] * max_len_in_batch
        flag = 1
        for s in range(len(tokens)):
            if tokens[s] == tokens_ans[0]:
                for t in range(len(tokens_ans)):
        ...
        try:
            ...
            print(self._tril_matrix.size())
            print(second_st)
            print(second_end)
            print(e)

        return (input_ids, segment_ids, answer_tag, position_ids, input_mask)
