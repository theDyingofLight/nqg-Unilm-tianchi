...
def main():
    ...
    # Other parameters
    my_parser.add_argument("--max_seq_length", default=128, type=int,
    ...
    my_parser.add_argument("--eval_batch_size", default=64, type=int,
    ...
    my_parser.add_argument("--no_cuda", action='store_true',
                           help="Whether not to use CUDA when available")
    my_parser.add_argument("--local_rank", type=int, default=-1,
    ...
    my_parser.add_argument('--fp16', action='store_true',
                           help="Whether to use 16-bit float precision instead of 32-bit")
    my_parser.add_argument('--fp16_opt_level', type=str, default='O1',
    ...
    my_parser.add_argument("--mask_prob_eos", default=0, type=float,
                           help="Number of prediction is sometimes less than max_pred when sequence is short.")
    my_parser.add_argument('--max_pred', type=int, default=69,
    ...
    my_logger.info("device: {} n_gpu: {}, distributed training: {}, 16-bits training: {}".format(
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs) + 1, desc="Epoch",
            ...
            for step, batch in enumerate(iter_bar):
                ...
                masked_lm_loss = model(input_ids, segment_ids, answer_tag, input_mask, lm_label_ids,
                ...
                for step, batch in enumerate(iter_dev):
                    with torch.no_grad():
                        batch = [
                            t.to(device) if t is not None else None for t in batch]
                        input_ids, segment_ids, answer_tag, input_mask, lm_label_ids, masked_pos, masked_weights, _ = batch
                        masked_dev_loss = model(input_ids, segment_ids, answer_tag, input_mask, lm_label_ids,
                                                masked_pos=masked_pos, masked_weights=masked_weights)
